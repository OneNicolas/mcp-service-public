...
export async function comparerCommunes(args: ComparerCommunesArgs): Promise<ToolResult> {
  ...
  if (!communes || communes.length < 2) {
    return {
      content: [{ type: "text", text: "Veuillez fournir au moins 2 communes \u00e0 comparer (maximum 5)." }],
      isError: true,
    };
  }
  ...
  try {
    ...
    for (let i = 0; i < resolvedResults.length; i++) {
      ...
      if (result.status === "fulfilled" && result.value) {
        ...
        errors.push(`"${communes[i]}" : commune non trouv\u00e9e`);
      }
    }

    if (resolved.length < 2) {
      return {
        content: [{ type: "text", text: `Impossible de r\u00e9soudre suffisamment de communes.\n${errors.join("\n")}` }],
        isError: true,
      };
    }
    ...
    if (communeDataList.length < 2) {
      return {
        content: [{ type: "text", text: "Impossible de r\u00e9cup\u00e9rer les donn\u00e9es pour suffisamment de communes." }],
        isError: true,
      };
    }
...
async function fetchCommuneData(nom: string, code: string): Promise<CommuneData> {
  const [reiResult, dvfAppartResult, dvfMaisonResult, zonageResult] = await Promise.allSettled([
    fetchREI(code),
...
function buildComparisonReport(data: CommuneData[], errors: string[]): string {
  ...
  lines.push(`| Prix m\u00e9dian/m\u00b2 appart. | ${data.map((d) => d.prixM2Appart ? formatEuro(d.prixM2Appart) : "N/A").join(" | ")} |`);
  lines.push(`| Prix m\u00e9dian/m\u00b2 maison | ${data.map((d) => d.prixM2Maison ? formatEuro(d.prixM2Maison) : "N/A").join(" | ")} |`);
  ...
  lines.push(`| Intercommunalit\u00e9 | ${data.map((d) => d.intercommunalite ?? "N/A").join(" | ")} |`);
  lines.push("");
  ...
  if (withTFB.length >= 2) {
    ...
    lines.push(`  \ud83c\udfc6 Taxe fonci\u00e8re la plus basse : **${minTFB.nom}** (${minTFB.tauxTFB} %)`);
  }
  ...
  if (withMaison.length >= 2) {
    ...
    lines.push(`  \ud83c\udfc6 Maisons les moins ch\u00e8res : **${minPrix.nom}** (${formatEuro(minPrix.prixM2Maison!)}/m\u00b2)`);
  }
  lines.push("");

  if (errors.length > 0) {
    lines.push(`\u26a0\ufe0f Communes non trouv\u00e9es : ${errors.join(", ")}`);
    lines.push("");
  }

  lines.push("_Sources : DGFiP REI via data.economie.gouv.fr, DVF via data.gouv.fr, zonage ABC Minist\u00e8re Transition \u00e9cologique_");
  return lines.join("\n");
}
